...
export default function Movies() {
  ...
  async function getMoviesApi(page) {
    let { data } = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?api_key=ee3d9c0552ad42f229b5ca281a29f904&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}`
    );
  ...
  async function search(ee) {
    if (ee.target.value === "") {
      ...
      let { data } = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=ee3d9c0552ad42f229b5ca281a29f904&language=en-US&query=${ee.target.value}&page=1`
      );
